...
- invoices (id uuid PK, invoice_number varchar, document_type varchar [invoice/credit_note], credit_note_of_id uuid FK→invoices, credit_reason text, contract_id uuid FK→contracts, customer_id uuid FK→customers, invoice_date date, due_date date, billing_period_from date, billing_period_to date, status varchar [draft/issued/sent/paid/overdue/cancelled], subtotal_net numeric, total_vat numeric, total_gross numeric, currency varchar, notes text, created_at timestamptz)
